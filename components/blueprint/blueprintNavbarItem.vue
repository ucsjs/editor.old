<template>
    <ol>
        <li v-for="(item, key) in items" :key="key" class="flex flex-col rounded-md" @click="openedState[key] = !openedState[key]"> 
            ...
            <div class="flex-row w-full" v-if="openedState[key]">
                <ol class="ml-4">
                    ...
                    >
                        <div>
                            <font-awesome-icon :icon="headerIcon(subitem.metadata.type)" v-if="openedState[key]" />
                        </div>
